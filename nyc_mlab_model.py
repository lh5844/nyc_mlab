...
plt.scatter(df['hour_of_day'], df['MeanThroughputMbps'], alpha=0.3, label='Raw Data Points')
plt.scatter(pred_hourly_avg['hour_of_day'], pred_hourly_avg['y_pred'], color='red', marker='o', label='Predicted Throughput for Single Point')
plt.xlabel('Hour of Day')
plt.ylabel('Mean Throughput (Mbps)')
plt.title('NYC Internet Throughput vs Hour of Day (Data Points)')
plt.xticks(range(0,24))
plt.grid(True, alpha=0.3)
plt.legend()
plt.savefig("results/nyc_throughput_raw_vs_predicted.png", dpi=300, bbox_inches='tight')
...
plt.grid(True, alpha=0.3)
plt.legend()
plt.savefig("results/nyc_throughput_trends.png", dpi=300, bbox_inches='tight')